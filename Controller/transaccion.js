...
class Transaccion
{
    ...
    gettransaccion = async ( req=request, res=response ) => {
        ...
        try {
            let {id} = req.params
            const caja = await transaccionModel.findById(id);
            res.status(200).json({
                status:200,
                msg:caja
            })
    ...
    getAlltransaccion = async ( req=request, res=response ) => {
        ...
        try {
            const caja = await transaccionModel.find();
            res.status(200).json({
                status:200,
                msg:caja
            })
    ...
    posttransaccion = async ( req=request, res=response ) => {
        ...
        try {
            ...
            let caja = new transaccionModel({boleta,estatus,total,metodo})
    ...
    puttransaccion = async ( req=request, res=response ) => {
        ...
        try {
            ...
            await transaccionModel.findByIdAndUpdate(id, {total,metodo});
            res.status(200).json({
                status:200,
                msg:"OK"
            })
